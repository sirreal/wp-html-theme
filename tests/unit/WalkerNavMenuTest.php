...
class WalkerNavMenuTest extends TestCase {
	...
	public function test_start_el_passes_url_and_title_through_escape_functions(): void {
		...
		Monkey\tearDown();
		Monkey\setUp();
		Functions\when('esc_url')->alias(static function ($u) {
			return '[U:' . $u . ']';
		});
		Functions\when('esc_html')->alias(static function ($h) {
			return '[H:' . $h . ']';
		});
		...
		$this->assertSame('<li><a href="[U:https://example.com/?x=1&y=2]">[H:A & B]</a>', $output);
	}
}
