...
8. **`alignwide` / `alignfull` front-end widths (2026-04-30, resolved)**: surfaced during the dequeue-rollback testing — `alignwide` did not extend past content width, and `alignfull` did not reach viewport edges. Resolved in two parts: (a) `add_theme_support('align-wide')` and `add_editor_style('style.css')` landed alongside the Theme Check round (see deviation #9) so the editor toolbar surfaces wide/full options and the editor canvas matches the front end; (b) the front-end CSS to actually realize the widths landed as a follow-up restructure of `style.css` to the constrained-layout pattern (article capped at `wideSize`, article's direct children default to `contentSize`, `.alignwide` releases to `wideSize`, `.alignfull` breaks out to viewport via `100vw` / `50%-50vw`). The CSS uses `--content-size` / `--wide-size` custom properties that mirror `theme.json`'s `layout.contentSize` / `wideSize`; the values are duplicated across the two configs (mild redundancy accepted; alternative would be parsing `theme.json` in PHP, heavier than the savings warrant).
...
9. **Theme Check RECOMMENDED triage (2026-04-30)**: re-ran Theme Check after the dequeue rollback. Items addressed:
   ...
   Items deliberately skipped (and why): `register_block_style`, `register_block_pattern`, `add_theme_support('wp-block-styles')` — block-theme transition is an explicit non-goal, and `wp-block-styles` would re-introduce `wp-block-library-theme`. `add_theme_support('custom-header')` and `add_theme_support('custom-background')` — the theme uses neither. "No sidebars/widget areas" — deliberate non-goal.
