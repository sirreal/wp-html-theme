...
8. **Open follow-up (2026-04-30)**: `alignwide` does not extend past content width — needs `add_theme_support('align-wide')` and `theme.json` `layout.contentSize`/`wideSize`. Surfaced during the dequeue-rollback testing; tracked as a follow-up rather than rolled into the same change. **Resolved in deviation #9.**

9. **Theme Check RECOMMENDED triage (2026-04-30)**: re-ran Theme Check after the dequeue rollback. Items addressed:
   - `add_theme_support('align-wide')` added in `inc/setup.php` (resolves #8). `theme.json` already declares `wideSize`.
